...
any third-party [Express.js](https://ghub.now.sh/express_app) middleware.

Or try the
[examples/with-express_app.js](https://github.com/node-formidable/node-formidable/blob/master/examples/with-express_app.js)
...
app.get('/', (req, res) => {
  res.send(`
    <h2>With <code>"express_app"</code> npm package</h2>
    <form action="/api/upload" enctype="multipart/form-data" method="post">
      <div>Text field title: <input type="text" name="title" /></div>
